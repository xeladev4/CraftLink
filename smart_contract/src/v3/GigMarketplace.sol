...
contract GigMarketplace {
    ...
    function applyForGigFor(address _artisan, bytes32 _databaseId) external onlyRelayer {
        ...
        require(craftCoin.allowance(_artisan, address(this)) >= requiredCFT, "Insufficient allowance");
    ...
    function closeGigFor(address _client, bytes32 _databaseId) external onlyRelayer {
        uint256 thisGigId = indexes[_databaseId];
        GigInfo storage gig = gigs[thisGigId];
    ...
    function hasAppliedForGig(address _artisan, bytes32 _databaseId) external view returns (bool) {
        ...
        return _isApplicant(thisGigId, _artisan);
    }
